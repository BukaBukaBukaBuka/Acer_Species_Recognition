...
[Pcont, Pener, Pcont1, Pener1, Pcont2, Pener2] = extract(pierwsza);
[Dcont, Dener, Dcont1, Dener1, Dcont2, Dener2] = extract(druga);
[Tcont, Tener, Tcont1, Tener1, Tcont2, Tener2] = extract(trzecia);
...
X = X(:); Y = Y(:);
C = classify([X Y], [x' y'], Allgroups, 'quadratic');

axes(handles.axes2);
gscatter(x, y, Allgroups, 'rky', 'v^o')
hold on; 
gscatter(X, Y, C, 'kry', '.', 1, 'off');
%plot(w, h, 'kp', 'MarkerSize', 12);
title('Klasyfikacja wybranych nowotworów mózgu');
xlabel('Kontrast'); xlabel('Energia'); 
hold off;
...
[class] = classify([ener cont ener1 cont1 ener2 cont2], [Acont' Aener' Acont1' Aener1' Acont2' Aener2' Agroups'], Agroups, 'quadratic');
...
axes(handles.axes2);
gscatter(x, y, Agroups, 'rky', 'v^o')
hold on; 
gscatter(X, Y, C, 'kry', '.', 1, 'off');
plot(kont, ener, 'kp', 'MarkerSize', 12);
title('Klasyfikacja wybranych nowotworów mózgu');
xlabel('Kontrast'); xlabel('Energia');
hold off;
